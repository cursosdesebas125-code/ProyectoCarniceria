...
from app.use_cases.apply_payment import ApplyPaymentUseCase
...
async def create_cliente(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Could not create client: {exc}",
        )
...
async def list_clientes(
    ...
    total_pages = max(1, (total_count + limit - 1) // limit) if limit > 0 else 0
...
@router.post("/{cliente_id}/abonar")
async def abonar_deuda(
    cliente_id: int,
    schema: AbonoRequest,
    ...
    """
    Apply a FIFO payment (abono) to a customer's pending orders
    and update their outstanding debt.
    """
    use_case = ApplyPaymentUseCase(
        clientes_repo=clientes_repo,
        pedidos_repo=pedidos_repo,
        detalles_repo=detalles_repo,
    )
    try:
        return await use_case.execute(cliente_id, schema)
...
async def get_cliente_statement(
    ...
    """Get a detailed account statement for a client."""
